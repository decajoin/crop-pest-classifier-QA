...
class BaseInferencer:
    ...
    2. Forward the data to the mmpretrain by :meth:`forward`. ``BaseInferencer``
       assumes the mmpretrain inherits from :class:`mmengine.checkpoints.BaseModel` and
       will call `mmpretrain.test_step` in :meth:`forward` by default.
    ...
    Args:
        model (BaseModel | str | Config): A mmpretrain name or a path to the config
            file, or a :obj:`BaseModel` object. The mmpretrain name can be found
            by ``cls.list_models()`` and you can also query it in
            :doc:`/modelzoo_statistics`.
        pretrained (str, optional): Path to the checkpoints. If None, it will
            try to find a pre-defined weight from the mmpretrain you specified
            (only work if the ``mmpretrain`` is a mmpretrain name). Defaults to None.
        ...
        **kwargs: Other keyword arguments to initialize the mmpretrain (only work if
            the ``mmpretrain`` is a mmpretrain name).
    """
    ...
    def preprocess(self, inputs: InputType, batch_size: int = 1, **kwargs):
        """Process the inputs into a mmpretrain-feedable format.

        Customize your preprocess by overriding this method. Preprocess should
        return an iterable object, of which each item will be used as the
        input of ``mmpretrain.test_step``.
    ...
    def forward(self, inputs: Union[dict, tuple], **kwargs):
        """Feed the inputs to the mmpretrain."""
    ...
    def visualize(self,
        ...
        Args:
            ...
            preds (Any): Predictions of the mmpretrain.
    ...
    def postprocess(
        ...
        **kwargs,
        ...
        Args:
            preds (List[Dict]): Predictions of the mmpretrain.
    ...
    def list_models(pattern: Optional[str] = None):
        """List checkpoints defined in metafile of corresponding packages.
