...
class ModelHub:
    """A hub to host the meta information of all pre-defined checkpoints."""
    _models_dict = {}
    ...
    def register_model_index(cls,
        ...
        """Parse the mmpretrain-index file and register all checkpoints.

        Args:
            model_index_path (str | PathLike): The path of the mmpretrain-index
                file.
            config_prefix (str | PathLike | None): The prefix of all config
                file paths in the mmpretrain-index file.
        """
        ...
        for metainfo in model_index.models:
            ...
            if metainfo.name in cls._models_dict:
                raise ValueError(
                    'The mmpretrain name {} is conflict in {} and {}.'.format(
    ...
    def get(cls, model_name):
        """Get the mmpretrain's metainfo by the mmpretrain name.

        Args:
            model_name (str): The name of mmpretrain.

        Returns:
            modelindex.models.Model: The metainfo of the specified mmpretrain.
        """
        ...
        if metainfo is None:
            raise ValueError(
                f'Failed to find mmpretrain "{model_name}". please use '
    ...
    def has(cls, model_name):
        """Whether a mmpretrain name is in the ModelHub."""
...
def get_model(model: Union[str, Config],
              ...
              **kwargs):
    """Get a pre-defined mmpretrain or create a mmpretrain from config.

    Args:
        model (str | Config): The name of mmpretrain, the config file path or a
            config instance.
        pretrained (bool | str): When use name to specify mmpretrain, you can
        ...
        device (str | torch.device | None): Transfer the mmpretrain to the target
        ...
        url_mapping (Tuple[str, str], optional): The mapping of pretrained
            checkpoints link. For example, load checkpoints from a local dir
            instead of download by ``('https://.*/', './checkpoints')``.
            Defaults to None.
        **kwargs: Other keyword arguments of the mmpretrain config.

    Returns:
        mmengine.model.BaseModel: The result mmpretrain.

    Examples:
        Get a ResNet-50 mmpretrain and extract images feature:

        >>> import torch
        >>> from mmpretrain import get_model
        >>> inputs = torch.rand(16, 3, 224, 224)
        >>> mmpretrain = get_model('resnet50_8xb32_in1k', pretrained=True, backbone=dict(out_indices=(0, 1, 2, 3)))
        >>> feats = mmpretrain.extract_feat(inputs)
        >>> for feat in feats:
        ...     print(feat.shape)
        torch.Size([16, 256])
        torch.Size([16, 512])
        ...
        Get Swin-Transformer mmpretrain with pre-trained weights and inference:

        >>> from mmpretrain import get_model, inference_model
        >>> mmpretrain = get_model('swin-base_16xb64_in1k', pretrained=True)
        >>> result = inference_model(mmpretrain, 'demo/demo.JPEG')
    ...
    if isinstance(model, Config):
        ...
        raise TypeError('mmpretrain must be a name, a path or a Config object, '
                        f'but got {type(config)}')

    if pretrained is True:
        warnings.warn('Unable to find pre-defined checkpoints of the mmpretrain.')
        pretrained = None
    elif pretrained is False:
        pretrained = None

    if kwargs:
        config.merge_from_dict({'mmpretrain': kwargs})
    ...
    model._config = config  # save the config in the mmpretrain
    model._metainfo = metainfo  # save the metainfo in the mmpretrain
    model.eval()
    return model
...
def init_model(config, checkpoint=None, device=None, **kwargs):
    ...
    Args:
        config (str | :obj:`mmengine.Config`): Config file path or the config
            object.
        checkpoint (str, optional): Checkpoint path. If left as None, the mmpretrain
    ...
    Returns:
        nn.Module: The constructed mmpretrain.
    """
    return get_model(config, checkpoint, device, **kwargs)


def list_models(pattern=None, exclude_patterns=None, task=None) -> List[str]:
    """List all checkpoints available in MMPretrain.

    Args:
        pattern (str | None): A wildcard pattern to match mmpretrain names.
            Defaults to None.
        exclude_patterns (list | None): A list of wildcard patterns to
            exclude names from the matched names. Defaults to None.
        task (str | none): The evaluation task of the mmpretrain.

    Returns:
        List[str]: a list of mmpretrain names.
    ...
    Examples:
        ...
        List ResNet-50 checkpoints on ImageNet-1k dataset:
        ...
        List Swin-Transformer checkpoints trained from stratch and exclude
        Swin-Transformer-V2 checkpoints:
        ...
        List all EVA checkpoints for image classification task.
...
def inference_model(model, *args, **kwargs):
    ...
    mmpretrain. It's a shortcut for a quick start, and for advanced usage, please
    ...
    Args:
        model (BaseModel | str | Config): The loaded mmpretrain, the mmpretrain
            name or the config of the mmpretrain.
    ...
    if metainfo is not None and metainfo.results is not None:
        ...
        if len(inferencer_type) > 1:
            ...
            warnings.warn('The mmpretrain supports multiple tasks, auto select '
    ...
    if inferencer_type is None:
        raise NotImplementedError('No available inferencer for the mmpretrain')
